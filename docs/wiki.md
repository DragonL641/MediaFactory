...
| M2M100-1.2B | Encoder-Decoder | 机器翻译 | 1.2B |
...
│       MediaFactory 使用 safetensors 格式加载 M2M100-1.2B       │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
|------|-------|------|------|----------|------------|
| M2M100-1.2B | 1.2B | FP16 | ~9.9 GB | ~5 GB | ~4.8 GB | 8 GB |
...
#### 翻译模型架构：Encoder-Decoder vs Decoder-Only

翻译模型有两大架构流派，理解它们的区别是选型的基础。

**Encoder-Decoder (Seq2Seq) — 翻译专用模型**

```
┌─────────────────────────────────────────────────────────────┐
│              Encoder-Decoder 翻译流程                         │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  源语言: "Hello world"                                       │
│         │                                                   │
│         ▼                                                   │
│  ┌───────────────────────┐                                  │
│  │     Encoder (读懂)     │  双向注意力，完整理解原文          │
│  │  每个词都能看到上下文    │                                  │
│  └───────────┬───────────┘                                  │
│              │ Context Vector                                │
│              ▼                                              │
│  ┌───────────────────────┐                                  │
│  │     Decoder (写出)     │  逐词生成，关注 Encoder 输出      │
│  │  你好 → 世界 → </s>    │                                  │
│  └───────────────────────┘                                  │
│                                                             │
│  特点: 两个网络各司其职，输入输出天然分离                      │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

- 两个独立网络：Encoder 负责读懂原文，Decoder 负责生成译文
- 输出确定性强：给定相同输入，输出几乎相同
- 无需 prompt：直接指定源语言和目标语言即可
- 同等参数量下，翻译质量优于 Decoder-Only
- 代表模型：M2M100、NLLB、MarianMT

**Decoder-Only (Causal LM) — 通用语言模型**
...
│  │              同一个网络 (文本续写)                       │  │
│  │                                                       │  │
│  │  "将以下英文..." → "Hello world" → 你好 → 世界 → </s>  │  │
│  │       ↑ 读的部分         ↑ 写的部分(逐词预测下一个)      │  │
│  │                                                       │  │
│  │  只能看左边，不能看右边(因果注意力)                      │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
│  特点: 一个网络同时负责读和写，本质是"接龙"                    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

- 单一网络：通过文本续写实现翻译（本质是"接龙"）
- 输出有随机性：temperature、top_p 等参数影响结果
- 需要 prompt：翻译效果依赖 prompt 工程
- 通用性强：同一个模型可以做翻译、摘要、问答等
- 代表模型：GPT、LLaMA、腾讯混元翻译 HY-MT1.5

**为什么同等参数量下 Encoder-Decoder 翻译更优？**

| 维度 | Encoder-Decoder | Decoder-Only |
|------|-----------------|--------------|
| **编码能力** | Encoder 专门做双向理解 | 只能单向（从左到右）理解原文 |
| **参数利用率** | 100% 参数用于翻译 | 参数同时服务于理解和生成，存在"分心" |
| **训练效率** | 只需翻译语料 | 需要海量通用语料 |
| **研究共识** | 1.2B 参数 ≈ 7B+ Decoder-Only 翻译质量 | 需要更大模型才能匹敌 |

---

#### 候选翻译模型对比

MediaFactory 对比了以下候选模型：

| 模型 | 架构 | 参数量 | 许可证 | 磁盘占用 | CJK 质量 | 限制 |
|------|------|--------|--------|----------|----------|------|
| **M2M100-1.2B** | Encoder-Decoder | 1.2B | **MIT** | ~4.8 GB | 良好 | — |
| NLLB-200-1.3B | Encoder-Decoder | 1.3B | CC-BY-NC-4.0 | ~5 GB | 优秀 | **禁止商用** |
| HY-MT1.5-1.8B | Decoder-Only | 1.8B | 腾讯自定义 | ~3.6 GB | 优秀 | **排除 EU/UK/Korea 地区** |

**许可证详解**：

- **MIT**：无任何限制，可自由商用、修改、分发
- **CC-BY-NC-4.0**：非商用许可，商业使用需要单独授权
- **腾讯自定义**：允许商用但排除欧盟/英国/韩国地区，有地域合规风险

---

#### MediaFactory 选型：M2M100-1.2B
...
│   └── HY-MT 地域限制 → 排除
│
├── 2. 架构优势 — Encoder-Decoder 翻译专用
│   ├── 1.2B 参数即可匹敌 7B+ Decoder-Only 的翻译质量
│   └── 输出确定性强，适合批量处理
│
├── 3. 生态兼容 — HuggingFace 原生支持
│   ├── transformers 直接加载，无需适配
│   ├── safetensors 格式，安全高效
│   └── 100+ 语言覆盖，CJK 表现良好
│
└── 4. 部署简单
    ├── 单模型文件，无量化需求
    └── FP16 精度即可高效推理（~5 GB 显存）
```
...
| 特性 | 本地模型 (M2M100-1.2B) | 远端 LLM (GPT-4, Claude 等) |
|------|---------------------|---------------------------|
...
```bash
# 直接执行获取版本号
python mediafactory/_version.py
# 输出: 0.2.0
```
...
| CI/CD | `python mediafactory/_version.py` | 简单直接 |
...
│  ├── components/  通用组件                                   │
│  ├── pages/       页面组件                                   │
│  │   ├── Tasks/       任务管理（创建/编辑/批量处理）           │
│  │   └── Settings/    设置（模型/LLM供应商/参数配置）           │
...
React 组件
    ...
    │ WebSocket (实时进度)
    ▼
FastAPI 后端 (127.0.0.1:8765)
    │
    ▼
TaskExecutor → Service → Pipeline → Engine
```
...
const createTask = useMutation({
  mutationFn: (config) => apiClient.createTask(config),
...
#### Q4: 为什么 M2M100 使用 Safetensors 而不是 GGUF？

**答**：
1. **架构限制**：M2M100 是 Encoder-Decoder 架构
2. **工具链不成熟**：Python 生态暂无成熟的 M2M100 GGUF 加载方案
...
- [M2M100 Paper](https://arxiv.org/abs/2010.10982)
*   **算法创新**：Facebook M2M100 使用创新的 Many对多语言直接翻译架构
*   **工具链不成熟**：Python 生态暂无成熟的 Encoder-Decoder GGUF 加载方案
*   **transformers 原生支持**：Safetensors 可直接通过 HuggingFace 加载，无需转换格式

...
class ASSEngine:
    ...
    # 说话人颜色调色板（BGR 格式，用于 ASS）
    SPEAKER_COLORS = {
        "SPEAKER_00": "&H00FFFFFF",  # 白色
        "SPEAKER_01": "&H0000FFFF",  # 黄色
        "SPEAKER_02": "&H00FF0000",  # 蓝色
        "SPEAKER_03": "&H0000FF00",  # 绿色
        "SPEAKER_04": "&H00FF00FF",  # 青色
        "SPEAKER_05": "&H000000FF",  # 红色
    }
    # 备用颜色（说话人数量超出预定义颜色时使用）
    _FALLBACK_COLORS = [
        "&H0080FF80",  # 浅绿
        "&H00FF8000",  # 紫色
        "&H00008080",  # 深黄
    ...
    def _build_ass_content(
        ...
        # 检测说话人并生成动态样式
        speaker_styles = self._build_speaker_styles(segments, styles.get("Default", self.DEFAULT_STYLE))
        ...
        # 写入原有样式（Default、Secondary 等）
        for style_name, style in styles.items():
            style_line = self._format_style(style_name, style)
            lines.append(style_line)

        # 写入说话人动态样式
        for speaker_name, speaker_style in speaker_styles.items():
            style_line = self._format_style(speaker_name, speaker_style)
            lines.append(style_line)
        ...
        for segment in segments:
            ...
            # 确定使用的样式名：有说话人标签时使用说话人样式，否则使用 Default
            style_name = speaker if (speaker and speaker in speaker_styles) else "Default"
            ...
            if bilingual and original_text:
                if layout == BilingualLayout.TRANSLATE_ON_TOP:
                    ...
                    # 再写主字幕（译文），使用说话人样式
                    lines.append(self._format_dialogue(start, end, style_name, text))
                elif layout == BilingualLayout.ORIGINAL_ON_TOP:
                    # 原文在上（主字幕），译文在下（副字幕）
                    lines.append(self._format_dialogue(start, end, "Secondary", text))
                    lines.append(
                        self._format_dialogue(start, end, style_name, original_text)
                    )
    ...
    def _build_speaker_styles(
        ...
        检测 segments 中出现的所有说话人，为每个说话人生成一个
        基于 base_style 的样式，仅替换 PrimaryColour 为该说话人的颜色。

        Args:
            segments: 字幕分段列表
            base_style: 基础样式配置（通常是 Default 样式）

        Returns:
            说话人样式字典，key 为说话人 ID（如 "SPEAKER_00"），
            value 为样式配置字典。无说话人时返回空字典。
        """
        # 收集所有唯一说话人（保持出现顺序）
        seen_speakers = set()
        unique_speakers = []
        ...
        for speaker in unique_speakers:
            ...
            elif fallback_idx < len(self._FALLBACK_COLORS):
                color = self._FALLBACK_COLORS[fallback_idx]
                fallback_idx += 1
            else:
                color = "&H00FFFFFF"  # 最终兜底：白色
